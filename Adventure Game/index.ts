...
// Enemy Data
let enemies: string[] = ["Skeleton", "Zombie", "Warrior", "Assassin", "Vampire"];
let maxEnemyHealth: number = 75;
let maxEnemyAttackDamage: number = 25;

// Player Data
let playerHealth: number = 100;
let maxPlayerAttackDamage: number = 50;
let numHealthPots: number = 3;
let healthPotHealAmount: number = 30;
let healthPotDropChance: number = 50;   // ==> 50% chance to drop a health potion whenever a enemy dies!
...
while(running){
    ...
    while(enemyHealth > 0){
        ...
        if(answer.option == "Attack"){
            let damageDealt: number = Math.floor(Math.random() * maxPlayerAttackDamage);
            let damageTaken: number = Math.floor(Math.random() * maxEnemyAttackDamage); 
